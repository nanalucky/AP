...
public class BS_Marker_Manager : MonoBehaviour {
	...
	List<BS_Main_Health> _Critical_Targets = new List<BS_Main_Health>();
			...
			if (_markers [i].HitCheck () != null) {
					...
					HitFlesh = true;
						...
						if(_markers [i]._target[t].GetComponent<BS_Limb_Hitbox>() != null)
						{
							...
							if(_markers [i]._target[t].GetComponent<BS_Limb_Hitbox>().CriticalHitBox == true)
							{
								_Critical_Targets.Add(_markers [i]._target[t].GetComponent<BS_Limb_Hitbox>().MainHealth);
							}
				...
				{ 
					...
					if( _Targets_Raw_Hit[i2]!= null && _Targets_Raw_Hit[i2].GetComponent<BS_Main_Health> () != null && _Used_Targets.Contains(_Targets_Raw_Hit[i2]) == false)
					{
						...
						TargetRawHealth.Bloodflood (_Blade_Direction[i2], _Blade_Startpoint[i2]);
							...
							if(_Critical_Targets.Contains(TargetRawHealth) == false)
							{
								TargetRawHealth.ApplyDamage (_damage);
							}
							if(_Critical_Targets.Contains(TargetRawHealth) == true)
							{
								TargetRawHealth.ApplyDamage (_damage * TargetRawHealth.CriticalMultiplier);
							}
